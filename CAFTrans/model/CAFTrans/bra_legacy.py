...
class TopkRouting(nn.Module):
    ...
    """
    def __init__(self, qk_dim, topk=4, qk_scale=None, param_routing=False, diff_routing=False, n=1):
        super().__init__()
        self.topk = topk
        self.qk_dim = qk_dim
        self.scale = qk_scale or qk_dim ** -0.5
        self.diff_routing = diff_routing
        self.weight = nn.Parameter(torch.ones(n, 256, 256))
    ...
    def forward(self, query:Tensor, key:Tensor)->Tuple[Tensor]:
        ...
        attn_logit = attn_logit * self.weight
...
class KVGather(nn.Module):
    def __init__(self, mul_weight='none'):
        ...
        self.down = nn.Conv2d(4608, 1536, kernel_size=1, padding=0)
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        out = rearrange(out, '(n j i) m (h w) c -> n (j h) (i w) (m c)', j=self.n_win, i=self.n_win,
                        h=16//self.n_win, w=16//self.n_win)
